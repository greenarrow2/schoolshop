...
DIV.yahoo2 A.prev:hover {
    ...
    <body>
        <div class="main_bestln">
            ...
            <div class="toolbar">
                <div class="w1210">
                    <span class="fl" id="memberInfo">您好gege，欢迎来到乐农优选！ 
...
$(function(){
    ...
    <div class="curtUrl mt5">
        <b><a href="http://www.bestln.com/">首页</a></b><c_rt:forEach items="${types}" var="type" varStatus="vs"><span>&gt;</span>
        <c_rt:if test="${vs.index==0}"><a href="${pageContext.request.contextPath }/front/product/getProductAndOneType.htm?id=${type.id}"></c_rt:if>
        <c_rt:if test="${vs.index==1}"><a href=""></c_rt:if>
        ${type.name}</a>
                </c_rt:forEach>
                </div>
    <div class="lists">
        <div class="lLeft fl">
                    ...
                    <div class="on">
                        <b></b>
                        <a href="">${typeTwo.name}</a>
                    </div>
                    <ol class="">
                                        <c_rt:forEach items="${typeTwo.children}" var="tp">
                                                <li><a href="">${tp.name}</a></li>
                                         </c_rt:forEach>        
                                            </ol>
                </li>
                        <c_rt:forEach items="${typeTwos}" var="type">
                                <li>
                    
                    <div class="">
                        <b></b>
                        <a href="${pageContext.request.contextPath }/front/product/getProductAndTwoType.htm?id=${type.id}">${type.name}</a>
                    </div>
                    <ol class="none">
        ...
        <div class="lRight fr">
               ...
               <!--  <span class="fr">共<b>10</b>个商品,<b>1</b>/1</span> -->
            </div>
	        <div id="product">
	                      <div class="goods">
	                <ul>
            ...
            <c_rt:if test="${pg.totalRecord>=20}">
             <div class="PageList">
                       ...
                       <c_rt:if test="${pg.currentPage!=1}">
  						<li class="previous"><a href="javascript:page(${typeTwo.id},${(pg.currentPage-2)*pg.maxResult},20)">上一页</a></li>
  						</c_rt:if>
                       <c_rt:if test="${pg.currentPage==1}">
  						<li class="previous"><a href="javascript:page(${typeTwo.id},0,20)">上一页</a></li>
  						</c_rt:if>
  						<c_rt:forEach begin="${pg.pageIndex.startIndex}" end="${pg.pageIndex.endIndex}" var="wp">
  						<c_rt:if test="${pg.currentPage==wp}">
  						<li class="page selected"><a href="javascript:page(${typeTwo.id},${(wp-1)*pg.maxResult},20)">${wp}</a></li>
  						</c_rt:if>
  						<c_rt:if test="${pg.currentPage!=wp}">
  						<li class="page"><a href="javascript:page(${typeTwo.id},${(wp-1)*pg.maxResult},20)">${wp}</a></li>
  						</c_rt:if>
  						</c_rt:forEach>
  						<c_rt:if test="${pg.currentPage!=pg.totalPage}">
  						<li class="next"><a href="javascript:page(${typeTwo.id},${(pg.currentPage)*pg.maxResult},20)">下一页</a></li>
 						</c_rt:if>
  						<c_rt:if test="${pg.currentPage==pg.totalPage}">
  						<li class="next"><a href="javascript:page(${typeTwo.id},${(pg.currentPage-1)*pg.maxResult},20)">下一页</a></li>
 						</c_rt:if>
 						</ul>                
             </div>
             </c_rt:if> 
