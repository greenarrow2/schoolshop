...
@Controller
@RequestMapping("front/cartmanage")
public class ManageCartController {
	...
	@ResponseBody
	public  DataThreeResult  addCart(Integer productId,Integer styleId,Integer num,
		...
		if(productId!=null && productId > 0)
		{
			...
			result.setObj(product.getSellPrice());
			result.setData(buyCart.getTotalSellPrice());
		}
		return result;
	}
	...
	@ResponseBody
	public DataThreeResult  decCart(Integer productId,Integer styleId,Integer num,
			HttpServletRequest request,HttpServletResponse response) {
		//首先从当前的session中找到是否有购物车(主要是考虑到用户是否会打开一个新的窗口)
		DataThreeResult result = new DataThreeResult();
		BuyCart buyCart = (BuyCart)request.getSession().getAttribute("buycart");
	...
	public String  deleteCartItem(Integer productId,Integer styleId,
			HttpServletRequest request,HttpServletResponse response) {
		//首先从当前的session中找到是否有购物车(主要是考虑到用户是否会打开一个新的窗口)
		BuyCart buyCart = (BuyCart)request.getSession().getAttribute("buycart");
		if(buyCart!=null)
		...
		return "../../front/cart/cartProducts";
	}
	/**
	 * 购物车中所有的商品
	 * @return
	...
	public String allCartProducts(HttpServletRequest request,HttpServletResponse response){
		...
		request.getSession().setAttribute("buycart", buyCart);
		request.setAttribute("totalSellPrice", buyCart.getTotalSellPrice());
		request.setAttribute("orderTotalPrice", buyCart.getOrderTotalPrice());
		request.setAttribute("totalSavePrice", buyCart.getTotalSavePrice());
		request.setAttribute("deliveFee", buyCart.getDeliveFee());
		return "../../front/order/createOrder";
	}
}
