...
<%@include file="/front/share/lib.jsp" %>
            ...
            <c_rt:if test="${pg.totalRecord>=20}">
             <div class="PageList">
                       ...
                       <c_rt:if test="${pg.currentPage==1}">
  						<li class="previous"><a href="javascript:oneTypePage(${id},0,20)">上一页</a></li>
  						</c_rt:if>
  						<c_rt:forEach begin="${pg.pageIndex.startIndex}" end="${pg.pageIndex.endIndex}" var="wp">
  						<c_rt:if test="${pg.currentPage==wp}">
  						<li class="page selected"><a href="javascript:oneTypePage(${id},${(wp-1)*pg.maxResult},20)">${wp}</a></li>
  						</c_rt:if>
  						<c_rt:if test="${pg.currentPage!=wp}">
  						<li class="page"><a href="javascript:oneTypePage(${id},${(wp-1)*pg.maxResult},20)">${wp}</a></li>
  						</c_rt:if>
  						</c_rt:forEach>
  						<c_rt:if test="${pg.currentPage!=pg.totalPage}">
  						<li class="next"><a href="javascript:oneTypePage(${id},${(pg.currentPage)*pg.maxResult},20)">下一页</a></li>
 						</c_rt:if>
  						<c_rt:if test="${pg.currentPage==pg.totalPage}">
  						<li class="next"><a href="javascript:oneTypePage(${id},${(pg.currentPage-1)*pg.maxResult},20)">下一页</a></li>
 						</c_rt:if>
 						</ul>                
             </div>
             </c_rt:if>
